...
model_3 <- brm(
  formula = log_consumption_std ~ Year + Company_Size_z +
    ...
    (1 | Locality_id) +
    (1 | nace_section_letter:Year) +        # NACE × Year
    (1 | nace_section_letter:Locality_id) + # NACE × Locality
    (1 | Year:Locality_id),                 # Year × Locality
  data = db,
  family = gaussian(),
  prior = c(
...
var_pars_m3 <- c(
  ...
  "sd_nace_section_letter:Year__Intercept",
  "sd_Year:Locality_id__Intercept",
  "sigma"
)
...
for (p in fixed_pars_m3) {
  ...
  print(g)
  ggsave(paste0("traceplot_m3_", p, ".pdf"), g, width = 4, height = 3.5)
}
...
for (q in var_pars_m3) {
  ...
  print(g)
  ggsave(paste0("traceplot_m3_", q, ".pdf"), g, width = 4, height = 3.5)
}
...
for (p in fixed_pars_m3) {
  ...
  print(g)
  ggsave(paste0("posterior_dist_m3_", p, ".pdf"), g, width = 4, height = 3.5)
}
...
for (q in var_pars_m3) {
  ...
  print(g)
  ggsave(paste0("posterior_dist_m3_", q, ".pdf"), g, width = 4, height = 3.5)
}


# Graphical Posterior Predicitve Checks
p1 <- pp_check(model_3, type = "dens_overlay", ndraws = 100) +
  clean_plot_theme 
p2 <- pp_check(model_3, type = "stat", stat = "sd", ndraws = 100) +
  clean_plot_theme 
p3 <- pp_check(model_3, type = "stat", stat = "max", ndraws = 100) +
  clean_plot_theme 
p4 <- pp_check(model_3, type = "intervals", ndraws = 100) +
  clean_plot_theme 
p5 <- pp_check(model_3, type = "stat", stat = "mean", ndraws = 100) +
  clean_plot_theme 

# Saving
ggsave("/Users/christinegrech/Desktop/ppc_comparing_obs_rep_m3.pdf", p1, width = 6, height = 4)
ggsave("/Users/christinegrech/Desktop/ppc_sd_m3.pdf",           p2, width = 6, height = 4)
ggsave("/Users/christinegrech/Desktop/ppc_max_m3.pdf",          p3, width = 6, height = 4)
ggsave("/Users/christinegrech/Desktop/ppc_intervals_m3.pdf",    p4, width = 6, height = 4)
ggsave("/Users/christinegrech/Desktop/ppc_mean_m3.pdf",          p5, width = 6, height = 4)



# Descriptive statistics on the posterior distribution of M3
results <- describe_posterior(model_3, effects = "all", component = "all", ci = 0.89, ci_method = "hdi", test = "p_direction")

# Filtering to see the Interaction Standard Deviations
print(results[grep("sd_", results$Parameter), ])



### Model 3A (Considering only the meaningful interactions) ###
model_3A <- brm(
  formula = log_consumption_std ~ Year + Company_Size_z +
    (1 | Company_id) +
    (1 | nace_section_letter) +
